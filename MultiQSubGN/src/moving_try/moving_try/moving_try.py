...
class Moving(Node):
    def __init__(self):
        ...
        #Substitute variables and nodes
        self.declare_parameter('isSubstitute', False)
        self.isSubstitute = self.get_parameter('isSubstitute').value
        self.SUB = self.get_parameter('isSubstitute').value
        self.subCoor = (0.0,0.0)


        self.substitute_sub = self.create_subscription(
            Float32MultiArray, 'failed', self.qTable_callback, 10)
        self.substitute_pub = self.create_publisher(
            Float32MultiArray, 'failed', 10)
        ...
        self.substitute_subscriber = self.create_subscription(
            Float32MultiArray, 'substitute', self.coordinates_callback, 10)
        ...
        # Loading already saved files if necessary
        '''
        with open('/home/coders/MultiQSub/Exp/'+self.fileName+'QTable.pkl', 'rb') as f:
            self.qTable = pickle.load(f)
        
        
        with open('/home/coders/MultiQSub/Exp/'+self.fileName+'.pkl', 'rb') as f:
            self.historyList = pickle.load(f)


        with open('/home/coders/MultiQSub/Exp/'+self.fileName+'QEntries.pkl', 'rb') as f:
        ...
        # Give a take off command but wait for the delay to start the first iteration
    ...
    def search(self):
        ...
        if self.start_pos:
            ...
            if self.SUB: # Substitute idle reset
                ...
                with open('/home/coders/MultiQSub/Exp/'+self.fileName+'QTable.pkl', 'wb') as f:
                    pickle.dump(self.qTable, f)
            
                with open('/home/coders/MultiQSub/Exp/'+self.fileName+'QEntries.pkl', 'wb') as f:
        ...
        if rand > targetP:
            ...
            if self.hidder == indexS:
                ...
                with open('/home/coders/MultiQSub/Exp/'+self.fileName+'.pkl', 'wb') as f:
            ...
            m = Float32MultiArray()
            m.data = self.qTable.reshape(-1)
            self.substitute_pub.publish(m)
    ...
    def timer_callback(self):
        ...
        # Get target
        if self.isSubstitute:
            targetX = self.subCoor[0]
            targetY = self.subCoor[1]
        else:
    ...
    def reset_subscribe_callback(self,msg):
        if self.start:
            ...
            if not self.finder and not self.isSubstitute:
    ...
    def coordinates_callback(self,msg):
        if self.isSubstitute:
            self.target = False
            self.subCoor = msg.data

    # Q-Table transmission
    def qTable_callback(self,msg):
        if self.isSubstitute:
            table = msg.data
            table = np.array(table)
            table = table.reshape((255,8))
            noise = np.random.normal(0, 1, size=(255,8))

            self.qTable = self.qTable * 0.6 + table * 0.4 + noise
